...
const prep = await prepareBeaconSafeSwap({ amountInUnits: amount, recipient, wallet: recipient });
console.log(JSON.stringify(prep, null, 2));
if (!prep.ok) process.exit(1);
console.log("executing...");
const res = await executeBeaconSafeSwap({ amountInUnits: amount, recipient, wallet: recipient });

/**
 * Image job against live Render API — prove SVG creative ships.
 */
...
async function main() {
  ...
  const created = await api<{ jobId: string }>("/v1/jobs", {
    ...
    body: JSON.stringify({
      serviceId: "image",
      briefText: "Minimal mint Beacon mark on warm paper. Flat vector thumbnail for social.",
    }),
  });
  ...
  const nonce = ("0x" + createHash("sha256").update(created.jobId + Date.now()).digest("hex")) as `0x${string}`;
  ...
  if (image) {
    const content = await api<{ content: string | null; mimeType: string }>(
      `/v1/jobs/${created.jobId}/artifacts/${image.id}`,
    );
    console.log(
      "image",
      content.mimeType,
    ...
    console.log("NO image artifact");
  }

  const lock = await escrow.locks(jobHash);
  console.log("final", final, { released: lock[2], refunded: lock[3] });
  if (!image) throw new Error("image artifact missing");

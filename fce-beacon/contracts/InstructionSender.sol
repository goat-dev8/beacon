...
contract HelloWorldInstructionSender {
    ...
    /// @notice Bound Work — Sealed Fit evaluation.
    // forge-lint: disable-next-line(unsafe-typecast)
    bytes32 public constant OP_TYPE_FIT = bytes32("FIT");
    // forge-lint: disable-next-line(unsafe-typecast)
    bytes32 public constant OP_COMMAND_EVALUATE = bytes32("EVALUATE");
    /// @notice Bound Work — acceptance evaluation.
    // forge-lint: disable-next-line(unsafe-typecast)
    bytes32 public constant OP_TYPE_JOB = bytes32("JOB");
    // forge-lint: disable-next-line(unsafe-typecast)
    bytes32 public constant OP_COMMAND_ACCEPT = bytes32("ACCEPT");
    ...
    /// @notice Sends a FIT/EVALUATE instruction (JSON payload).
    function sendEvaluateFit(bytes calldata _payload) external payable {
        address[] memory teeIds = TEE_MACHINE_REGISTRY.getRandomTeeIds(_getExtensionId(), 1);
        address[] memory cosigners = new address[](0);
    ...
    /// @notice Sends a JOB/ACCEPT instruction (JSON payload).
    function sendAccept(bytes calldata _payload) external payable {
        address[] memory teeIds = TEE_MACHINE_REGISTRY.getRandomTeeIds(_getExtensionId(), 1);
        address[] memory cosigners = new address[](0);

        ITeeExtensionRegistry.TeeInstructionParams memory params = ITeeExtensionRegistry.TeeInstructionParams({
            opType: OP_TYPE_JOB,
            opCommand: OP_COMMAND_ACCEPT,
            message: _payload,
            cosigners: cosigners,
            cosignersThreshold: 0,
            claimBackAddress: msg.sender

...
import { generateTextContent } from "./textGenerate.js";

export { isAcceptableTextDeliverable, isStubDeliverable } from "./textGenerate.js";
...
export const PIPELINE_CAPS = {
  version: "2026-08-08-agent-jobs-real-sol",
  imageSvg: true,
  ...
  videoMotionPan: true,
  flareRequired: true,
  /** Text/coding jobs must ship AgentRouter output — never scaffold stubs. */
  textSoftFail: false,
...
async function generateContent(job: PipelineJob): Promise<StageArtifact[]> {
  return generateTextContent(job);
}
...
async function composeDeliverable(
  ...
  const draftMeta = inputs.find((a) => a.kind === "draft")?.meta ?? {};
  const modelNote =
    typeof draftMeta.model === "string"
      ? ` · model \`${draftMeta.model}\``
      : typeof draftMeta.provider === "string"
        ? ` · ${draftMeta.provider}`
        : "";
  await writeFile(
    packPath,
    `${body.trim()}\n\n---\n\n_Delivered by Beacon · service \`${job.serviceId}\`${modelNote}_\n`,
    "utf8",
  );
